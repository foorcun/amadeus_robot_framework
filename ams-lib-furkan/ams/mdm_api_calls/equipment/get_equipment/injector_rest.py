...
def injector(kwargs, session_key="defaultKey"):
    ...
    endpoint_type = kwargs.get("endpoint_type", "equipmentType")
    path_param = kwargs.get("equipmentTypeId", None)
    ...
    equipment_endpoints = (
        context_data.get("end_points", {}).get("mdm").get("equipment", {})
    )

    endpoint = (
        f"{equipment_endpoints.get(endpoint_type)}/{path_param}"
        if path_param
        else equipment_endpoints.get(endpoint_type)
    )
    ...
    LOGGER.debug("REST Details for GET equipment type details: %s", rest_details)
