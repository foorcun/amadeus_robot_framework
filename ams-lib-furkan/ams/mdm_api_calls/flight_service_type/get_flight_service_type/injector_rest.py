...
def injector(kwargs, session_key="defaultKey"):
    ...
    flight_service_type = (
        context_data.get("end_points", {}).get("mdm", {}).get("flight_service_type", {})
    )
    endpoint = flight_service_type.get(endpoint_type)
    ...
    LOGGER.debug("REST Details for Flight Service Type : %s", rest_details)
