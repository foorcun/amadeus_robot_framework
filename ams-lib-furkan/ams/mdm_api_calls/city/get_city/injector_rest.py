...
def injector(kwargs, session_key="defaultKey"):
    ...
    city_endpoint = context_data.get("end_points", {}).get("mdm").get("city", {})
    endpoint = city_endpoint.get(endpoint_type)
    ...
    LOGGER.debug("REST Details for the city details: %s", rest_details)
