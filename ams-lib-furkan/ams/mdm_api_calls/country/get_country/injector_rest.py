...
def injector(kwargs, session_key="defaultKey"):
    ...
    country_endpoint = context_data.get("end_points", {}).get("mdm").get("country", {})
    endpoint = country_endpoint.get(endpoint_type)
    ...
    LOGGER.debug("REST Details for the country details: %s", rest_details)
